...
local function on_input_enter(input)
    if not input.tooltip then return end

    GameTooltip:Hide()
    GameTooltip:ClearLines()
    GameTooltip:SetOwner(input, "ANCHOR_TOPLEFT")

    for _, v in pairs(input.tooltip) do GameTooltip:AddLine(v) end
    GameTooltip:Show()
end
...
local function hex_to_argb(hex)
    if string.len(hex) == 8 then
        return tonumber("0x" .. string.sub(hex, 1, 2)) / 255,
               ...
               tonumber("0x" .. string.sub(hex, 7, 8)) / 255
    else
        return 1, tonumber("0x" .. string.sub(hex, 1, 2)) / 255,
               tonumber("0x" .. string.sub(hex, 3, 4)) / 255,
               tonumber("0x" .. string.sub(hex, 5, 6)) / 255
    end
end
...
local function rgba_to_hex(r, g, b, a)
    ...
    if a == nil then return string.format("FF%02x%02x%02x", r, g, b) end
...
-- ---------------------------------------------------------------------------------------------------------------------
function config_gui.create_checkbutton(name, text, checked, on_change, tooltip,
                                       parent, template)
    local checkbox_frame = CreateFrame("Frame", nil, parent)

    -- input
    local checkbox = CreateFrame("CheckButton", nil, checkbox_frame, template)
    ...
    if not checkbox.Text then
        checkbox.Text = parent:CreateFontString(nil, 'ARTWORK', 'GameFontHighlightLeft')
    ...
    checkbox:SetHitRectInsets(0, -checkbox_frame:GetWidth(), 0, 0)
...
function config_gui.create_checkbox(name, text, checked, on_change, tooltip,
                                    parent)
    return config_gui.create_checkbutton(name, text, checked, on_change,
                                         tooltip, parent,
                                         "InterfaceOptionsCheckButtonTemplate")
end

function config_gui.create_radiobutton(name, text, checked, on_change, tooltip,
                                       parent)
    return config_gui.create_checkbutton(name, text, checked, on_change,
                                         tooltip, parent,
...
-- ---------------------------------------------------------------------------------------------------------------------
function config_gui.create_slider(text, on_change, min_value, max_value, step,
                                  value, tooltip, parent)
    -- input
    local slider_frame = CreateFrame("Frame", nil, parent)
    slider_frame:SetWidth(300)
    slider_frame:SetHeight(50)

    local slider_label = slider_frame:CreateFontString(nil, "OVERLAY",
                                                       "GameFontHighlight")
    ...
    slider:SetScript("OnValueChanged", function(frame)
        local val = floor((frame:GetValue() - min_value) / step + 0.5) * step +
                        min_value

        if val < min_value then val = min_value end

        if val > max_value then val = max_value end
    ...
    local min_text = slider:CreateFontString(nil, "ARTWORK",
    ...
    local max_text = slider:CreateFontString(nil, "ARTWORK",
                                             "GameFontHighlightSmall")
...
-- ---------------------------------------------------------------------------------------------------------------------
function config_gui.create_color_picker(name, text, value, has_opacity,
                                        on_change, tooltip, parent)
    ...
    picker_frame:SetScript("OnClick", function()
        local on_color_picker_change = function(restore)
            ...
            if restore then
                ...
                new_a, new_r, new_g, new_b = OpacitySliderFrame:GetValue(),
        ...
        ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc =
            on_color_picker_change, on_color_picker_change,
            on_color_picker_change;
    ...
    -- text
    local color_text = picker_frame:CreateFontString(nil, "OVERLAY",
                                                     "GameFontHighlight")
